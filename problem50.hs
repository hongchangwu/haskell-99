...
huffman xs = sortBy (comparing fst) . 
             encode .
             htree . 
             Heap.fromList $ [HItem (w, Leaf x) | (x, w) <- xs]
  where htree ys = case Heap.uncons ys of
          Just (HItem (w1, t1), ys') -> 
            case Heap.uncons ys' of 
              Nothing -> t1
              Just (HItem (w2, t2), ys'') -> htree $ Heap.insert (HItem (w1 + w2, Branch t1 t2)) ys''
        encode (Branch l r) = [(x, '0':code) | (x, code) <- encode l] ++
