...
spantree (Graph ns es) = filter (liftA2 (&&) complete allconnected) trees
  where
    ...
    acc e@(u,v) gs = [Graph ((if u `elem` ns' then [] else [u]) ++
                             (if v `elem` ns' then [] else [v]) ++
                             ns') (e:es') | g@(Graph ns' es') <- gs,
                      not (connected g u v)] ++ gs
    connected (Graph _ es') u v  = v `elem` visit u []
      where
        visit x xs
          ...
          where
            f (u,v) xs = if u == x then visit v xs
                         else if v == x then visit u xs
                         else xs
